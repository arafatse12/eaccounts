...
<div class="wrapper">
  ...
  <?php 
      ...
      $total_invoice=$this->db->query("SELECT COUNT(*) as total FROM db_purchase where branch_id='$branch_id' ")->row()->total;
      /*Total Invoices Total*/
      $pur_total=$this->db->query("SELECT COALESCE(sum(grand_total),0) AS tot_pur_grand_total FROM db_purchase where branch_id='$branch_id'")->row()->tot_pur_grand_total;
      
      $tot_paid_amt=$this->db->select("COALESCE(SUM(paid_amount),0) AS paid_amount")->from("db_purchase")->where('branch_id',$branch_id)->get()->row()->paid_amount;
