...
<div class="wrapper">
 ...
 <?php 
      ...
      $q9=$this->db->query("SELECT COALESCE(SUM(b.qty*a.purchase_price),0) AS  opening_stock_price   FROM db_items AS a , db_stockentry AS b WHERE a.id=b.item_id and a.branch_id='$branch_id'");
      $opening_stock_price=$q9->row()->opening_stock_price;

      //total purchase amt
      $q4=$this->db->query("SELECT COALESCE(SUM(a.tax_amt),0) AS tax_amt FROM db_purchaseitems as a,db_purchase as b where a.purchase_id=b.id and b.purchase_status='Received' and  a.branch_id='$branch_id'");
      $purchase_tax_amt=$q4->row()->tax_amt;

      //total purchase amt
      $q1=$this->db->query("SELECT COALESCE(SUM(grand_total),0) AS pur_total FROM db_purchase where purchase_status='Received' and  branch_id='$branch_id'");
      $pur_total=$q1->row()->pur_total;
      $pur_total-=$purchase_tax_amt;

      //Other Charge of Purchase entry
      $q10=$this->db->query("SELECT COALESCE(SUM(other_charges_amt),0) AS other_charges_amt FROM db_purchase where purchase_status='Received'  and branch_id='$branch_id'");
      $pur_other_charges_amt=$q10->row()->other_charges_amt;

      //Disount purchase entry
      $q13=$this->db->query("SELECT COALESCE(SUM(a.discount_amt),0) AS discount_amt FROM db_purchaseitems as a,db_purchase as b where a.purchase_id=b.id and b.purchase_status='Received' and a.branch_id='$branch_id'");
      $purchase_discount_amt=$q13->row()->discount_amt;

      //if($purchase_discount_amt==0){
        $q14=$this->db->query("SELECT COALESCE(SUM(tot_discount_to_all_amt),0) AS tot_discount_to_all_amt FROM db_purchase where purchase_status='Received' and branch_id='$branch_id'");
        $purchase_discount_amt+=$q14->row()->tot_discount_to_all_amt;
      //}

      //purchase Paid Amount
      $purchase_paid_amount=$this->db->query("SELECT COALESCE(SUM(paid_amount),0) AS paid_amount FROM db_purchase where  branch_id='$branch_id'")->row()->paid_amount;
     ...
     // }
      ...
      //total sales amt
      $q2=$this->db->query("SELECT COALESCE(SUM(a.tax_amt),0) AS tax_amt FROM db_salesitems as a,db_sales as b where a.sales_id=b.id and b.sales_status='Final' and  a.branch_id='$branch_id'");
      $sales_tax_amt=$q2->row()->tax_amt;

      //Other Charge of Sales entry
      $q10=$this->db->query("SELECT COALESCE(SUM(other_charges_amt),0) AS other_charges_amt FROM db_sales where sales_status='Final' and branch_id='$branch_id'");
...
*/
      //if($sales_discount_amt==0){
        $q14=$this->db->query("SELECT COALESCE(SUM(tot_discount_to_all_amt),0) AS tot_discount_to_all_amt FROM db_sales where sales_status='Final' and branch_id='$branch_id'");
        $sales_discount_amt=$q14->row()->tot_discount_to_all_amt;
    //  }

      //Total SAles amount
      $query6="SELECT COALESCE(sum(grand_total),0) AS tot_sal_grand_total FROM db_sales where sales_status='Final' and branch_id='$branch_id'";
      $sal_total=$this->db->query($query6)->row()->tot_sal_grand_total;
      //$sal_total-=$sales_tax_amt;

      //sales Paid Amount
      $sales_paid_amount=$this->db->query("SELECT COALESCE(SUM(paid_amount),0) AS paid_amount FROM db_sales where branch_id='$branch_id'")->row()->paid_amount;
     ...
     // }
      ...
      //total expense amt
      $q3=$this->db->query("SELECT COALESCE(SUM(expense_amt),0) AS exp_total FROM db_expense where branch_id='$branch_id'");
    ...
    <section class="content">
           ...
           <div class="form-group col-md-4">
             <label for="to_date">Select Date</label>
                ...
                <div class="input-group">
                   <button type="button" class="btn btn-default pull-right daterange-btn" id="pl-daterange-btn">
                   <span>
      ...
      <div class="row">
        ...
        <div class="form-group">
                ...
                <div class="col-sm-3">
                <div class="input-group">
                  <button type="button" class="btn btn-default pull-right daterange-btn" id="pl2-daterange-btn">
                    <span>
                      <i class="fa fa-calendar"></i> Select Date Range
                    </span>
                    <i class="fa fa-caret-down"></i>
