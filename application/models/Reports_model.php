...
class Reports_model extends CI_Model {
	...
	public function show_sales_report(){
		...
		$this->db->select("a.id,a.sales_code,a.sales_date,b.customer_name,b.customer_code,a.grand_total,a.paid_amount,a.user_type,a.department_id,a.center_id");
	...
	public function get_profit_by_invoice(){
		...
		if($q1->num_rows()>0){
			...
			foreach ($q1->result() as $res1) {
				...
				$q2=$this->db->query("SELECT b.sales_qty,COALESCE(SUM(purchase_price*sales_qty),0) AS purchase_price, COALESCE(SUM(total_cost),0) AS total_cost FROM db_items AS a, db_salesitems AS b, db_sales AS c WHERE c.id=b.sales_id AND a.id=b.item_id and c.sales_status='Final' and a.branch_id=$branch_id
