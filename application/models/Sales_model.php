...
class Sales_model extends CI_Model {
	...
	var $column_order = array( 'a.return_bit','a.id','a.sales_date','a.sales_code','a.reference_no','a.grand_total','a.payment_status','a.created_by','b.customer_name','a.paid_amount','a.sales_status','a.pos','a.branch_id'); //set column field database for datatable orderable
	var $column_search = array('sales_due','a.return_bit','a.id','a.sales_date','a.sales_code','a.reference_no','a.grand_total','a.payment_status','a.created_by','b.customer_name','a.paid_amount','a.sales_status','a.pos','a.branch_id'); //set column field database for datatable searchable 
...
// for install payment
	    ...
	    if($command=='save'){//Create sales code unique if first time entry
		    ...
		    $sales_entry = array(
							...
							'interest_amount'           => $tot_round_off_amt,
		...
		//end
		if($installment){
			$installmentAmount= round(($tot_total_amt-$amount)/$installment_type);
			
			$q11=$this->db->query("select * db_installment_payments where sale_id='$sales_id'");
			if($q11){
				$q11=$this->db->query("delete from db_installment_payments where sale_id='$sales_id'");
			}
			for($i=1;$i<=$installment_type;$i++){

					$closing = $tot_total_amt-$amount;
				
					$closingAmount = ($closing-($installmentAmount*$i));
					if($i!=1){
						$opening = ($closing-($installmentAmount*($i-1)));
					}else{
						$opening = ($closing);
					}
					if($i==1 && $amount>0){
						$installment = array(
							'sale_id' 			 			=> $sales_id, 
							'schedule_number'               => 0,
							'installment_payment_month' 	=> $installment_type,
							'opening_balance'		 		=> $tot_total_amt, 
							...
							'paid_date' 	 				=> $CUR_DATE,
							'paid_date_time' 	 			=> $CUR_TIME,
						);
						
						$installmentPayment = $this->db->insert('db_installment_payments', $installment);
					}
					$day =$i*30;
					$date = date('d-m-Y',strtotime('+'.$day.' days',strtotime($CUR_DATE)));
					$installment = array(
						'sale_id' 			 			=> $sales_id, 
						'schedule_number'               => $i,
						'installment_payment_month' 	=> $installment_type,
			...
			for($i=1;$i<=$installment_type;$i++){
				...
				$day =$i*30;
				$date = date('d-m-Y',strtotime('+'.$day.' days',strtotime($CUR_DATE)));
				$principalAmount = round($installmentAmount-($tot_round_off_amt/$installment_type),2);
				$interestAmount = round(($tot_round_off_amt/$installment_type),2);
				$html .='<tr>'.
				...
				'<td class="text-center">'.($principalAmount).'</td>'.
				'<td class="text-center">'.($interestAmount).'</td>'.

				'<td class="text-center"> '.$installmentAmount.'</td>'.

				'<td class="text-center">'.($closingAmount).'</td>'.
			...
			$html2 .= '<div class="col-xs-12">
				<div class="col-sm-12">
				<a href="'.$base_url.'sales/print_schedule_invoice/'.$sales_id.'" target="_blank" class="btn btn-warning">
					<i class="fa fa-print"></i> 
				Installment Schedule Print
				</a>
				<a href="'.$base_url.'sales/installment_pdf/'.$sales_id.'" target="_blank" class="btn btn-primary">
					<i class="fa fa-file-pdf-o"></i> 
				Installment Schedule PDF
				</a>
				<a href="'.$base_url.'sales/invoice/'.$sales_id.'" target="_blank" class="btn btn-success">
		...
		return "success<<<###>>>$sales_id,$html,<<<####>>>$html2,sales>>>$sale_type";
	...
	public function get_details_sales(){
		//Validate This sales already exist or not
		$query=$this->db->query("select * from db_installment_payments");
		return $query;
	}
	...
	public function show_installment_pay_now_modal($sales_id){
		$q1=$this->db->query("select * from db_sales where id=$sales_id");
		$res1=$q1->row();
		$customer_id = $res1->customer_id;
		$q2=$this->db->query("select * from db_customers where id=$customer_id");
		...
		$paymentInstallment=$this->db->query("select * from db_installment_payments where sale_id=$sales_id and status='Due' order by id asc limit 1");
		$rows = $paymentInstallment->row();
		$installment = $rows->installment_amt;
		$date = $rows->installment_date;
		$schedule_number = $rows->schedule_number;
		?>
		<div class="modal fade" id="installment_pay_now">
		  <div class="modal-dialog ">
		    <div class="modal-content">
		      <div class="modal-header header-custom">
		        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
		          <span aria-hidden="true">&times;</span></button>
		        <h4 class="modal-title text-center">Installment Payments</h4>
		      </div>
		      <div class="modal-body">
		    ...
		    <div class="row">
		      <div class="col-md-12">
		      	<div class="row invoice-info">
			        ...
			        <div class="col-sm-4 invoice-col">
			          <b>Paid Amount :<span><?php echo number_format($paid_amount,2,'.',''); ?></span></b><br>
			          <b>Due Amount :<span id='due_amount_temp'><?php echo number_format($due_amount,2,'.',''); ?></span></b><br>
			          <b>Installment Month :<span ><?php echo ($schedule_number); ?></span></b><br>
		          ...
		          <div class="box box-solid bg-gray">
		            <div class="box-body">
		              <div class="row">
		         		<div class="col-md-4">
		                  <div class="">
		                  <label for="payment_date">Installment Date</label>
		                    <div class="input-group date">
			                      <div class="input-group-addon">
			                      <i class="fa fa-calendar"></i>
			                      </div>
			                      <input type="text" class="form-control pull-right datepicker" value="<?=$date; ?>" id="payment_date" name="payment_date" readonly>
			                    </div>
		                      <span id="payment_date_msg" style="display:none" class="text-danger"></span>
		                </div>
		               </div>
		                <div class="col-md-4">
		                  <div class="">
		                  <label for="amount">Amount</label>
		                    <input type="text" class="form-control text-right paid_amt" id="amount" name="amount" placeholder="" value="<?=$installment;?>" onkeyup="calculate_payments()">
		                      <span id="amount_msg" style="display:none" class="text-danger"></span>
		                </div>
		               </div>
		                <div class="col-md-4">
		      ...
		      <div class="modal-footer">
		        <button type="button" class="btn btn-default btn-lg" data-dismiss="modal">Close</button>
		        <button type="button" onclick="save_installment_payment(<?=$sales_id;?>)" class="btn bg-green btn-lg place_order btn-lg payment_save">Save<i class="fa  fa-check "></i></button>
		      </div>
		    </div>
		    <!-- /.modal-content -->
		  </div>
	...
	public function save_installment_payment(){
		extract($this->xss_html_filter(array_merge($this->data,$_POST,$_GET)));
		//print_r($this->xss_html_filter(array_merge($this->data,$_POST,$_GET)));exit();
    	if($amount=='' || $amount==0){$amount=null;}
		if($amount>0 && !empty($payment_type)){
			...
			$q3 = $this->db->insert('db_salespayments', $salespayments_entry);
			$schedule_number = $this->db->query("select schedule_number from db_installment_payments where sale_id=$sales_id and status='Due' order by id asc limit 1")->row()->schedule_number;
			$q7=$this->db->query("update db_installment_payments set 
			status='Paid',paid_amount='$amount',paid_date='$CUR_DATE',paid_date_time='$CUR_TIME' where sale_id='$sales_id' and schedule_number='$schedule_number'");
		}
		else{
			return "Please Enter Valid Amount!";
		}
