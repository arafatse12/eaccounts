...
class Dashboard_model extends CI_Model
{
	...
	public function breadboard_values()
	{
		...
		///Find total Products
		$query2="select coalesce(count(*),0) as tot_pro from db_items where branch_id='$branch_id'";
		...
		$tot_cust=$this->db->query($query3)->row()->tot_cust;	
  		...
  		$query4="SELECT COALESCE(COUNT(*),0) AS tot_pur FROM db_purchase where purchase_status='Received' and branch_id='$branch_id'";

		$tot_pur=$this->db->query($query4)->row()->tot_pur;	

  		//Total SAles Active
  		$query5="SELECT COALESCE(COUNT(*),0) AS tot_sal FROM db_sales where `sales_status`= 'Final' and branch_id='$branch_id'";
		...
		//Total expense amount
  		$query7="SELECT COALESCE(sum(expense_amt),0) AS tot_exp FROM db_expense where  branch_id='$branch_id'";
		$tot_exp=$this->db->query($query7)->row()->tot_exp;

		//Total SAles Due
		$query8="SELECT (COALESCE(sum(grand_total),0)-COALESCE(sum(paid_amount),0)) as sales_due FROM db_sales where `sales_status`= 'Final' and branch_id='$branch_id'";
		$sales_due=$this->db->query($query8)->row()->sales_due;
		//Total Purchase  Due
		$query9="SELECT (COALESCE(sum(grand_total),0)-COALESCE(sum(paid_amount),0)) as purchase_due FROM db_purchase where purchase_status='Received' and branch_id='$branch_id' ";
		$purchase_due=$this->db->query($query9)->row()->purchase_due;

		//Total SAles  Due
		$query10="SELECT COALESCE(sum(payment),0) as today_payment_received FROM db_salespayments where sales_id in (select id from db_sales where branch_id='$branch_id') and payment_date='".date("Y-m-d")."'";
		...
		$query9="SELECT (COALESCE(sum(grand_total),0)-COALESCE(sum(paid_amount),0)) as purchase_due FROM db_purchase where purchase_status='Received'  and branch_id='$branch_id'";
		$query9="SELECT (COALESCE(sum(grand_total),0)-COALESCE(sum(paid_amount),0)) as purchase_due FROM db_purchase where purchase_status='Received' and  branch_id='$branch_id'";
		$query12="SELECT COALESCE(sum(grand_total),0) AS todays_total_sales FROM db_sales where branch_id='$branch_id' and `sales_status`= 'Final' and sales_date='".date("Y-m-d")."' ";
		$todays_total_sales=$this->db->query($query12)->row()->todays_total_sales;

		//Total expense amount
  		$query13="SELECT COALESCE(sum(expense_amt),0) AS todays_total_expense FROM db_expense where  branch_id='$branch_id'  and expense_date='".date("Y-m-d")."'";
