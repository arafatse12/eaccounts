...
class Pos_model extends CI_Model {
	...
	public function get_details(){
		...
		extract($_POST);
	      ...
	      $q2=$this->db->query("select b.id as tax_id, a.*,b.tax,b.tax_name,a.tax_type,a.item_image from db_items a,db_tax b where a.branch_id='$branch_id' and b.id=a.tax_id and a.status=1 $str order by a.stock desc");
	      if($q2->num_rows()>0){
	...
	public function pos_save_update(){//Save or update sales
		...
		if($command=='update'){
			...
			$sales_entry = array(
		    				...
		    				'subtotal' 					=> $tot_amt,
		    				// 'round_off' 				=> $round_off,
							'interest_amount'           => $interest_rate_value,
		    				'grand_total' 				=> $tot_grand,
		    				/*System Info*/
		...
		if($installment_type){
			$installmentAmount= round(($tot_grand-$amount)/$installment_type);
		  ...
		  $q7 = $this->db->insert('db_salespayments', $salespayments_entry);
			for($i=1;$i<=$installment_type;$i++){
				$intAmount = $interest_rate_value;
					$closing = round($tot_grand-$amount);
					$installmentAmount= round(($tot_grand-$amount)/$installment_type);
					...
					$installment = array(
						...
						'interest_rate'					=> $interest_rate, 
						'interest_amount'		 		=> $intAmount,  
						'closing_balance' 		 		=> $installmentAmount,
						'paid_amount' 	 				=> NULL,
						'due_amount' 	 				=> $installmentAmount,
						'status' 	 					=> 'Due',
			...
			for($i=1;$i<=$installment_type;$i++){
				...
				$intAmount = $interest_rate_value;
				$monthInt =  round($intAmount/$installment_type,2);
				$day =$i*30;
				$date = date('d-m-Y',strtotime('+'.$day.' days',strtotime($CUR_DATE)));
				$principalAmount = round($installmentAmount-$monthInt,2);
				...
				'<td class="text-center">'.($monthInt).'</td>'.
			...
			for($i=1;$i<=$installment_type;$i++){
				...
				$day =$i*30;
				$date = date('d-m-Y',strtotime('+'.$day.' days',strtotime($CUR_DATE)));
				$intAmount = round((($tot_grand-$amount)*$interest_rate)/100,2);
				$monthInt =  round($intAmount/$installment_type,2);
