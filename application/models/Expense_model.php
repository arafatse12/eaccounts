...
class Expense_model extends CI_Model {
	...
	public function verify_and_save(){
		...
		$query1="insert into db_expense(branch_id,expense_code,category_id,expense_for,expense_amt,reference_no,note,created_date,created_time,created_by,status,system_ip,system_name,expense_date)
						values('$branch_id','$expense_code','$category_id','$expense_for','$expense_amt','$reference_no','$note','$CUR_DATE','$CUR_TIME','$CUR_USERNAME',1,'$SYSTEM_IP','$SYSTEM_NAME','".date("Y-m-d",strtotime($expense_date))."')";
	...
	public function get_details($id,$data){
		...
		$branch_id  = $this->session->userdata('branch_id');
		$query=$this->db->query("select * from db_expense where branch_id='$branch_id' and upper(id)=upper('$id')");
		if($query->num_rows()==0){
			show_404();exit;
		}
		else{
	...
	public function check_table_data($table_name,$field,$value){
		$branch_id  = $this->session->userdata('branch_id');
		return $this->db->query("select count(*) as tot_count from db_expense where branch_id='$branch_id' and $field='$value'")->row()->tot_count;
	}
