...
class Sales extends MY_Controller {
	...
	public function ajax_list()
	{
		...
		foreach ($list as $sales) {
			...
			$sales_type=$this->db->query("select sales_type  from db_sales where id='$sales->id'")->row()->sales_type;
			...
			$row[] = ucfirst($sales->created_by);
					...
					$str2 = '<div class="btn-group" title="View Account">
										...
										<ul role="menu" class="dropdown-menu dropdown-light pull-right">';
											...
											if($sales_type==3)
											$str2.='<li>
												<a title="Installment Pay" class="pointer" onclick="installment_pay_now('.$sales->id.')" >
													<i class="fa fa-credit-card text-blue"></i>Installment Payment Receive
												</a>
											</li>';
	...
	public function print_schedule_invoice($sales_id)
	{
		$data=$this->data;
		$data['page_title']=$this->lang->line('sales_invoice');
		$q4 = $this->db->query("select * from db_installment_payments where upper(sale_id)=upper('$sales_id')");
		foreach ($q4->result() as $key => $res) {
			$installment_payment_month = $res->installment_payment_month;
			$opening_balance = $res->opening_balance;
			$installment_amt = $res->installment_amt;
			...
			$paid_date_time = $res->paid_date_time;
		}
        $data=array_merge($data,array('sales_id'=>$sales_id,'installment_payment_month'=>$installment_payment_month,'opening_balance'=>$opening_balance,'installment_amt'=>$installment_amt,'interest_rate'=>$interest_rate,'interest_amount'=>$interest_amount,'closing_balance'=>$closing_balance,'paidAmount'=>$paidAmount,'due_amount'=>$due_amount,'status'=>$status,'paid_date'=>$paid_date));

		if(get_invoice_format_id()==3){
			$this->load->view('print-schedule-invoice',$data);
		}
	}
	...
	public function installment_pdf($sales_id){
	...
	public function show_installment_pay_now_modal(){
		$sales_id=$this->input->post('sales_id');
		echo $this->sales->show_installment_pay_now_modal($sales_id);
	}
	...
	public function save_installment_payment(){
		echo $this->sales->save_installment_payment();
	}
