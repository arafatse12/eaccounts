...
class Items extends MY_Controller {
	...
	public function get_json_items_details(){
		...
		//if (!empty($_GET['name'])) {
			...
			$sql =$this->db->query("SELECT id,item_name,item_code,stock FROM db_items where  branch_id=$branch_id  and status=1 and  (LOWER(item_name) LIKE '%$name%' or LOWER(item_code) LIKE '%$name%' or LOWER(custom_barcode) LIKE '%$name%')   limit 10");
