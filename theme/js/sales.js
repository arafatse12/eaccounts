        ...
        success: function(result) {
            ...
            if (result[0] == "success") {
                ...
                if (result3[1] == 3) {
                    ...
                    $('#installment_payment tbody').append(result2);
                    var salesId = result4[1].split(',')[0];
                    $('#pdf').append(salesId);

                    $('#saveId').hide();
                    // var salesId = result[1].split(',')[0];
                    // location.href = base_url + "sales/invoice/" + salesId;
                } else {
                    var salesId = result[1].split(',')[0];
                    location.href = base_url + "sales/invoice/" + salesId;
                }
...
function sales_data() {
    $.ajax({
        type: 'get',
        url: base_url + 'sales/get_details_sales',
        cache: false,
        contentType: false,
        processData: false,
        success: function(result) {
            ...
            console.log(result);
            if (result[0] == "success") {
                // var bb = base_url + "sales/invoice/" + result[1];
                // console.log(bb);

                $('#installment_payment tbody').append(result);
            } else if (result[0] == "failed") {
                toastr['error']("Sorry! Failed to save Record.Try again");
            } else {
                alert(result);
...
function installment_pay_now(sales_id) {
    $.post('sales/show_installment_pay_now_modal', { sales_id: sales_id }, function(result) {
        $(".installment_pay_now_modal").html('').html(result);
        //Date picker
        $('.datepicker').datepicker({
            autoclose: true,
            format: 'dd-mm-yyyy',
        ...
        $('#installment_pay_now').modal('toggle');
...
function save_installment_payment(sales_id) {
    var base_url = $("#base_url").val().trim();
    ...
    $(".box").append('<div class="overlay"><i class="fa fa-refresh fa-spin"></i></div>');
    $(".payment_save").attr('disabled', true); //Enable Save or Update button
    $.post('sales/save_installment_payment', { sales_id: sales_id, payment_type: payment_type, amount: amount, payment_date: payment_date, payment_note: payment_note }, function(result) {
        result = result.trim();
        //alert(result);return;
        if (result == "success") {
            $('#pay_now').modal('toggle');
