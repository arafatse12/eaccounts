...
function save(print = false, pay_all = false) {
    ...
    /* console.log("tot_grand="+tot_grand);
     ...
     return;*/
    if ($('[name=sale_type]').val() == '') {
        toastr["warning"]("Sale Type field is required");
        return;
    }
    ...
    $.ajax({
        ...
        success: function(result) {
            ...
            result5 = result2.trim().split("<<<####>>>");
            resultSales = result3.trim().split("sales>>>");
            var resultPdf = result5[1].split(',');
            var resultPdfType = resultSales[1].split('<<<###>>>');
            //return;
            if (result[0]) {
                if (result[0] == "success") {
                    var print_done = true;
                    $("#saveButton").addClass('hide');
                    if (resultPdfType[0] == 3) {
                        $("#paymenyId").removeClass('hide');
                        $('#installment_payment tbody').append(result[1]);
                        $('#pdf').append(resultPdf[0]);
...
$('#hold_invoice').click(function(e) {
    ...
    swal({
            title: "Hold Invoice ? Same Reference will replace the old list if exist!!",
            icon: "warning",
            buttons: true,
            dangerMode: true,
            content: {
